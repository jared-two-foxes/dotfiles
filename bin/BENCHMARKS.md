...
premium-request multiplier, not $/token - `lib/model-pricing.toml` intentionally
...
A separate follow-up pass ran every model in `lib/model-pricing.toml`'s "Free
...
- **A new model**: add pricing to `lib/model-pricing.toml` first if you want
