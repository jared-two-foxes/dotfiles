...
file, since that's literally what gets passed to it under the hood each
round). Stops early if propose-ticket-edit.py produces no revision (its
"no remaining work" case) - there's nothing left to loop on.

Usage:
    loop-ticket-review.py <ticket-id> [--model <model-id>]
                           [--max-iterations <n>] [--ticket-file-out <path>]
"""
