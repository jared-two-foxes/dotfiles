#!/usr/bin/env python3
"""
write-next-test - given a ticket (or an existing .gap-plan.md from a
prior check-ticket.py/resolve-ticket.py run), write a failing test for
the next acceptance criterion the codebase doesn't satisfy yet. No
implementation - that's a manual step that follows this one.

This is write-tests.py's --max-new 1 mode promoted to its own script
and the default behaviour (pass --max-new to write more than one, or
omit the cap entirely the same way write-tests.py does by default).

Re-entrant: fetch/plan/narrow are skipped if .ticket.md/.tdd-plan.md/
.gap-plan.md already exist and pass their validity check, and each
criterion already recorded in .gap-plan.md is never rewritten - it's
re-verified and skipped either way. See write-tests.py's docstring for
the full RED-GREEN-RED cycle this enables.

Compile-failure self-correction (explicitly breaks fail-fast): unlike
write-tests.py, a test that fails to compile is not an immediate,
unretriable failure here. The compile error is fed back to the Tester
agent for up to --max-compile-retries attempts total (the first write
counts as attempt 1) before giving up - on the theory that a single
criterion's test is small enough in scope that a model can usually
self-correct a syntax/type mistake given the actual compiler error,
and that's worth trying before bailing out to a human. If every attempt
still fails to compile, the run dies and shows the last compile
failure's output, same as write-tests.py's immediate-die behaviour.

Workflow this is part of: check-ticket.py (confirm work is needed) ->
write-next-test.py (this script, optional) -> implement-tests.py
(implement against the test this script wrote, optional) or manual
implementation -> validate-and-review.py (gate the result).

Usage:
    write-next-test <ticket-id> [--model <model-id>] [--config <path>]
                    [--max-new <n>] [--max-compile-retries <n>]
"""
...
DEFAULT_MAX_COMPILE_RETRIES = 3


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Write a failing test for the next acceptance criterion the "
    ...
    parser.add_argument(
        ...
        help="Stop after writing this many genuinely new tests (criteria "
             ...
             "Defaults to 1, so a bare run writes exactly the next test "
             "and stops - implement it to green, call again for the next. "
             "Pass a higher number (or omit the cap the way write-tests.py "
             "does) to write more than one per run.",
    )
    parser.add_argument(
        "--max-compile-retries",
        type=int,
        default=DEFAULT_MAX_COMPILE_RETRIES,
        help="Maximum attempts (including the first write) at getting one "
             "criterion's test to compile before giving up - each failed "
             "attempt's compiler error is fed back to the Tester agent for "
             "a fix. (default: %(default)s)",
    )
    parser.add_argument(
        "--ticket-file-in",
        type=Path,
    ...
    for criterion in criteria:
        ...
        if existing and lib.criterion_test_exists(*existing):
            ...
            # A prior run may have died at the compile gate after
            # annotating this criterion's test but before the suite
            # compiled cleanly. Re-check compilation here so resuming
            # after a manual fix doesn't silently skip a criterion whose
            # test still doesn't build.
        ...
        file_path, test_name, compile_result = lib.run_test_for_criterion_with_compile_retry(
            criterion, gap_plan_text, model, commands, max_attempts=args.max_compile_retries
        )

        if compile_result.returncode != 0:
            # file_path/test_name still get annotated below would be wrong
            # here - the test never reached a compiling state, so there's
            # nothing valid to record as covering this criterion.
            log_summary()
            lib.die_with_log(
                "compile",
                f"Tests still do not compile after {args.max_compile_retries} attempt(s) "
                f"(exit {compile_result.returncode}). See output above for the last failure.",
                criterion=criterion,
            )
